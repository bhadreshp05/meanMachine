...
// create a secret to create token with
var superSecret = config.secret;

// set the public folder to server public assets
app.use(express.static(__dirname + '/public'));

// set up our one route to index.html file
...
apiRouter.post('/authenticate', function (req, res) {
	...
	User.findOne({
		...
		// no user that username was found
		if (!user) {
			res.json({
				success: false,
			...
			if(!validPassword) {
				...
				// if use is found and password is right
				// create a token
				var token = jwt.sign	({
					name: user.name,
...
apiRouter.use(function (req, res, next) {
	...
	if (token) {
		...
		// return an HTTP response of 403 (accessed forbidden) and an error message
		return res.status(403).send({
			success: false,
